...
class RacingDashboard(QMainWindow):
    ...
    def initUI(self):
        ...
        self.layout.addWidget(self.velocimetro, 1, 0, 1, 1)

        # Adicionando a barra de combustível
        self.barra_combustivel = BarraCombustivelWidget()
        self.layout.addWidget(self.barra_combustivel, 1, 3)

        # Layout central para gráficos
    ...
    def update_velocimetro(self):
        # Simulação do velocímetro indo de 0 a 40 e voltando
        if self.velocimetro.valor_desejado >= 40:
            self.velocimetro.direcao = -1
        elif self.velocimetro.valor_desejado <= 0:
            self.velocimetro.direcao = 1

        nova_velocidade = self.velocimetro.valor_desejado + self.velocimetro.direcao * 2
        self.velocimetro.setVelocidade(nova_velocidade)
